...
export default function EditMatch() {
    ...
    (async () => {
      try {
        const response = await get(
          child(ref(db), `Partai/${router?.query.id}`)
        );
        console.log(response.val());
        setMatchData(response.val());
      } catch (error) {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      pemainMerah: matchData?.pemainmerah,
      kotaMerah: matchData?.kotamerah,
      pemainBiru: matchData?.pemainbiru,
      kotaBiru: matchData?.kotabiru,
      partai: matchData?.value,
    },
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        await set(ref(db, `Partai/${router.query.id}`), {
          label: values.partai,
          value: values.partai,
          pemainmerah: values.pemainMerah,
          kotamerah: values.kotaMerah,
